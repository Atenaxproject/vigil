    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <section className="mt-6 rounded-card border border-slate-200 bg-vigil-blue-light p-4">
        <div className="flex items-center justify-between gap-2">
          <p className="text-[13px] font-medium text-vigil-ink">{t('liveStatus.title')}</p>
          <button
            type="button"
            onClick={() => void fetchLive()}
            className="inline-flex items-center gap-1 text-[11px] text-vigil-blue"
        ...
        <p className="mt-2 font-mono text-[11px] text-vigil-muted">
        ...
        <a
          ...
          className="mt-2 inline-flex items-center gap-1 text-[13px] text-vigil-blue"
        >
      ...
      <section className="mt-8">
        <h2 className="text-[18px] font-semibold text-vigil-ink">{tc('stats.title')}</h2>
        <p className="mt-1 font-mono text-[11px] text-status-unverified">
      ...
      <section className="mt-10">
        <h2 className="text-[18px] font-semibold text-vigil-ink">{t('officialUpdates')}</h2>
        {loading && <div className="skeleton mt-4 h-24 rounded-card" />}
        {!loading && (liveData?.officialReports.length ?? 0) === 0 && (
          <p className="mt-3 text-[13px] text-vigil-muted">{t('noReports')}</p>
        )}
        <div className="mt-4 space-y-3">
          {liveData?.officialReports.map((report) => (
            ...
            >
              <p className="text-[13px] font-medium text-vigil-ink">{report.title}</p>
              <p className="mt-1 font-mono text-[11px] text-vigil-muted">
      ...
      <section className="mt-10">
        <h2 className="text-[18px] font-semibold text-vigil-ink">{tc('infra.title')}</h2>
        <p className="mt-1 text-[11px] text-vigil-muted">{t('infraLive')}</p>
        <div className="mt-4 space-y-3">
          {infra.length === 0 && (
            <p className="text-[13px] text-vigil-muted">{t('infraEmpty')}</p>
          )}
          {infra.map((item) => (
            <div
              key={item.id}
            ...
            >
              <div>
                ...
                {item.status_label && (
                  <p className="text-[11px] text-vigil-muted">{item.status_label}</p>
                )}
              </div>
      ...
      <section className="mt-10">
        <h2 className="text-[18px] font-semibold text-vigil-ink">{tc('hotlines.title')}</h2>
        <div className="mt-4 space-y-3">
          {hotlines.map((line) => (
      ...
      <section className="mt-10">
        <h2 className="text-[18px] font-semibold text-vigil-ink">{tc('sources.title')}</h2>
      ...
      <p className="mt-8 font-mono text-[11px] text-vigil-muted" suppressHydrationWarning>
