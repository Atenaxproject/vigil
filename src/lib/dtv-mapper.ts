...
export function mapDTVPersonaToFederated(person: DTVPersona): FederatedPerson {
  return {
    ...
    last_seen_location: person.ubicacion ?? person.estado_geo ?? 'Venezuela',
    // Geographic estado from ubicacion.estado — never contact-status `estado`
    estado: person.estado_geo ?? null,
    municipio: person.municipio ?? null,
    parroquia: person.parroquia ?? null,
