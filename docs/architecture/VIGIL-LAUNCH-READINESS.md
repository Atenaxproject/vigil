...
**[SHIPPED 2026-07-22 prompts 67 → 66 → 68]** Aftershock banner uses rolling 7d USGS (was crisis-pinned cumulative "20"); feed_health + stale markers; `/como-ayudar` expiry/suppress; `/monitor` global hazard relay with kill switch. Migrations `016_feed_health` + `017_hazard_events` **applied** (no further Orlando action on those).
